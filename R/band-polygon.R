...
#' draws nothing across such a gap. If the plot shades the *feasible* region the
#' gap is merely invisible; if it shades the *infeasible* region — as
...
#' because nothing was knocked out of it. A ring also closes itself, so a region
#' ending mid-scale gets a vertical edge instead of two curves trailing off.
#'
...
#' these are left in, since whether an isolated feasible point should be shown
...
#' @return A long-format tibble with `mean`, `y` and a `ring` id, ready for
#'   `geom_polygon(aes(group = ring))`. Zero rows if no row of `d` is finite.
#' @seealso [sd_region_data()], [plot_sd_region()].
#' @examples
#' # a reported alpha leaves stretches near each limit where no composite exists
#' d <- sd_region_data(
#'   0, 3, 7,
#'   rule = "alpha", n_items = 2, alpha = 0.70, by = 0.001
#' )
#' sum(is.na(d$lo))
#'
...
band_polygon <- function(d, by) {
  d <- d |>
    dplyr::filter(is.finite(.data$lo), is.finite(.data$hi)) |>
  ...
  unique(run) |>
    purrr::map(function(ring_current) {
      in_ring <- d[run == ring_current, , drop = FALSE]
      tibble::tibble(
        mean = c(in_ring$mean, rev(in_ring$mean), in_ring$mean[1]),
        y = c(in_ring$hi, rev(in_ring$lo), in_ring$hi[1]),
        ring = ring_current
      )
    }) |>
    purrr::list_rbind()
...
#' consistent tuple rather than to the reporting grid's step. Under
#' `granularity = "integer"` only the means an integer sum can round to survive,
#' so on the reporting grid almost every neighbouring pair of means is a gap;
#' splitting there would make each stripe its own one-column ring, and a
#' one-column ring has no width to draw. The median of the realised spacing is
#' the striping's own period, so the stripes join into the envelope and only a
#' true stretch of infeasible means still splits the region.
#'
#' The contour therefore claims less than the tuples do: it says which
#' `(mean, sd)` pairs are near the region the tests admit, not which are in it.
#'
...
#' @return A long-format tibble with `mean`, `y` and a `ring` id, as
#'   [band_polygon()] returns. Zero rows if no tuple is consistent.
...
umbrella_contour <- function(umbrella, by = NULL) {
  ...
  if (is.null(by)) {
    by <- if (nrow(envelope) > 1) stats::median(diff(envelope$mean)) else 1
  }

  band_polygon(envelope, by = by)
}
...
#' \eqn{s_{\max}^2 = \frac{n}{n-1}(u - m)(m - l)\,\delta}.
#' [sd_max_muilwijk()] computes the uncorrected arch and
#' [sd_max_structure_s()] the corrected maximum; this returns the correction
#' between them.
#'
...
#' and falls toward 0 as the mean approaches a scale limit.
#'
...
#' @export
sd_delta <- function(mean, n, lower, upper, tol = TOLERANCE) {
  span <- upper - lower
  count_lower <- n * (upper - mean) / span
  count_upper <- n * (mean - lower) / span
  frac_below <- function(x) {
    fractional <- x - floor(x)
    dplyr::if_else(fractional > 1 - tol, 0, fractional)
  }

  # sum of squares of the uncorrected bound, and of the sharp one
  muilwijk <- n * (upper - mean) * (mean - lower)
  sharp <- muilwijk - frac_below(count_lower) * frac_below(count_upper) * span^2

  dplyr::if_else(muilwijk <= 0, 1, pmax(0, sharp) / muilwijk)
}
